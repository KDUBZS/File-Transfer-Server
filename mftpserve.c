...
int main(int argc, char *argv[]){
	...
	listenfd = connection(PORT_NUMBER);
	listen(listenfd, 4);
	...
	while(1){
		...
		if(command[0] == 'D'){
			...
			int portnum;
			int childpid;
			unsigned int length1 = sizeof(struct sockaddr_in);

			memset(&data, 0, length);
			datalistenfd = connection(0);
			...
			write(controlfd, "No data connection made\n", 25);
		}else if(command[0] == 'G'){
			write(controlfd, "No data connection made\n", 25);
		}else if(command[0] == 'P'){
			write(controlfd, "No data connection made\n", 25);
	...
	while(1){
		...
		if(command[0] == 'L'){
			...
			if((lstat(path, s) == 0) && S_ISREG(s->st_mode)){
				...
				printf("Child %d... Sending %s to client\n", parentpid, path);
				...
				write(controlfd, "File not regular\n", 18);
				close(filefd);
				close(datafd);
				exit(EXIT_SUCCESS);
			}
			...
			write(controlfd, "A\n", 2);
			printf("Child %d... Receiving %s from client\n", parentpid, path);
